         ...
         MacDataDot16* dot16)
{
    ...
    else
    {
		...
		{
			dot16Bs->trigger.triggerType = DOT16e_TRIGEGER_METRIC_TYPE_CINR;
			...
			{
				dot16Bs->para.hoCinrTrigger = DOT16e_DEFAULT_HO_CINR_TRIGGER;
			}
			...
			{
				dot16Bs->para.hoCinrMargin = DOT16e_DEFAULT_HO_CINR_MARGIN;
			}
...
// Sleep mode statistics parameter
    ...
    if (node->isMihfEnabled == TRUE)
    {
    	sprintf(buf, "Number of Link_Going_Down indications sent = %d",
    	                    dot16Bs->stats.numLinkGoingDownIndSent);
    	IO_PrintStat(node,
    	                         "MAC",
    	                         "802.16e",
    	                         ANY_DEST,
    	...
    	sprintf(buf, "Number of Link_Parameters_Report indications sent = %d",
    	    	                    dot16Bs->stats.numLinkParametersReportInd);
    	    	IO_PrintStat(node,
    	    	                         "MAC",
    	    	                         "802.16e",
    	    	                         ANY_DEST,
                              ...
                              int startIndex)
{
    ...
    if (hoInd->mode == DOT16e_HOIND_Ho)
    {
        ...
        {
#ifdef ADDON_MIH
			bsId = (unsigned char *) &(macFrame[index]);
#endif
            index += DOT16_BASE_STATION_ID_LENGTH;
        }
    }
    ...
    if (ssInfo  && !ssInfo->inHandover)
    {
            ...
            hoIndHoMod->hoIndType == DOT16e_HOIND_ReleaseServBs)
        {
			...
			NodeAddress bsNodeId = MAPPING_GetNodeIdFromInterfaceAddress(node, MacHWAddressToIpv4Address(node, interfaceIndex, &macHWAddr));
			if (bsNodeId == node->nodeId)
				 MacDot16BsRemoveSsByCid(node, dot16, ssInfo->basicCid, TRUE);
			else
			{
#endif
				...
				ssInfo->ulCinrMean < dot16Bs->para.hoCinrTrigger)
	{
		...
		for (i = 0; i < DOT16e_DEFAULT_MAX_NBR_BS; i++)
		{
			if (ssInfo->nbrBsSignalMeas[i].bsValid &&
				...
				2 * DOT16_SS_DEFAULT_NBR_SCAN_MIN_GAP) <
				getSimTime(node)) &&
				ssInfo->nbrBsSignalMeas[i].dlMeanMeas.cinr > maxBsCinr)
			{
				found = TRUE;
				maxBsCinr =  ssInfo->nbrBsSignalMeas[i].dlMeanMeas.cinr;
				maxIndex = i;
			}
		}

		if (found &&
			(maxBsCinr - ssInfo->ulCinrMean > dot16Bs->para.hoCinrMargin))
		{
                                   ...
                                   PhySignalMeasurement* signalMea)
{
    ...
    ssInfo->lastUlMeaTime = getSimTime(node);
    	 ...
    	 //Send LinkParametersReport but after registering SS
#ifdef ADDON_MIH
    if (!ssInfo->firstTimerSent)
    {
    	ssInfo->timerSendPeriodicReport = MacDot16SetTimer(node,
							dot16,
							DOT16_TIMER_SendPeriodicReport,
							DOT16_TIMER_SEND_PERIODIC_REPORT_INTERVAL,
							NULL,
							ssInfo->basicCid);
    	ssInfo->firstTimerSent = TRUE;
    }
                    ...
                    const NodeInput* nodeInput)
{
    ...
    if (dot16->dot16eEnabled)
    {
...
#ifdef ADDON_MIH
        if (dot16Bs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_RSSI)
        {
#endif
        ...
        else if (dot16Bs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_CINR)
        {
        	dot16Bs->trigger.triggerValue =
        	                (signed char) DOT16e_DEFAULT_NBR_SCAN_CINR_TRIGGER;
        }
#endif                
        ...
        case MSG_MAC_TimerExpired:
        {
            ...
            switch (timerType)
            {
                ...
                case DOT16_TIMER_SendPeriodicReport:
                {
                	...
                	{
                	//if (!ssInfo->inHandover)
//                		MacDot16BsSendLinkParametersReportIndication(node, dot16, ssInfo);
					MESSAGE_Free(node, msg);
                	ssInfo->timerSendPeriodicReport = MacDot16SetTimer(node,
																	dot16,
																	DOT16_TIMER_SendPeriodicReport,
				...
				if (param_req->linkParamReq->elem->selector == 1) //this should be a switch
				{
					...
					{ //now we should build confirm primitive
						...
						if (found)
						{
							...
							paramValue.lpv = (LINK_PARAM_VAL) ssInfo->ulCinrMean;
							...
							Link_Get_Parameters_confirm(node, 0, LinkParametersStatusList, NULL, NULL, param_req->srcMihfID, node->nodeId, param_req->transactionId, param_req->transactionInitiated);
						}
						else
							Link_Get_Parameters_confirm(node, 1, NULL, NULL, NULL, param_req->srcMihfID, node->nodeId, param_req->transactionId, param_req->transactionInitiated); //STATUS = 1 - unknown failure
					}
