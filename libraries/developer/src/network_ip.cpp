...
#include "multicast_ces_srw_mospf.h"
...
#include "network_security_ces_isakmp.h"
...
#if defined(ADDON_BOEINGFCS) || defined(SNMP_INTERFACE)
...
// End for Dymo
...
#ifdef ADDON_MIH
	// MIH is not enabled by default
	ip->mihFunctionStruct = NULL;
#endif
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
        ...
        if (retVal)
        {
            //Starting range of ipFragUnit is changed to 64 instead of 256
            //to support the zigbee.
            ERROR_Assert(ipFragUnit >= 64 &&
...
#ifdef ADDON_MIH
	ip->pktBufSize = 0;
	std::queue<Message*>* inPacketBuf = new std::queue<Message*>;
	ip->packetBuf = inPacketBuf;
#endif
}
    ...
    switch (msg->protocolType)
    {
        ...
        case NETWORK_PROTOCOL_IP:
        {
            switch (msg->eventType)
            {
                ...
                case MSG_SPECTRUM_RealTimeAnalysis:
                {
                    NetworkDataIp *ip = (NetworkDataIp *)
                        node->networkData.networkVar;
                    ERROR_Assert(ip != NULL, "Unable to extract IP information\n");
                    int* interfaceIndex =
                        (int*)MESSAGE_ReturnInfo(msg,
                        INFO_TYPE_SpectrumInterfaceIndex);
                    SpectrumRealTimeAnalysis(node, *interfaceIndex);
                    MESSAGE_Send(node, msg,
                        ip->interfaceInfo[*interfaceIndex]->
                        manager->realTimeAnalysisTimer);
                    break;
                }

                /*case MSG_SPECTRUM_ChannelReAssign:
                {
                    SpectrumHandleMitigate(node, msg);
                    break;
                }*/

                case MSG_SPECTRUM_PredictiveTimeAnalysis:
                {
                    NetworkDataIp *ip = (NetworkDataIp *)
                        node->networkData.networkVar;
                    ERROR_Assert(ip != NULL, "Unable to extract IP information\n");
                    int* interfaceIndex = (int*)
                        MESSAGE_ReturnInfo(msg, INFO_TYPE_SpectrumInterfaceIndex);
                    SpectrumPredictiveTimeAnalysis(node, *interfaceIndex);
                    MESSAGE_Send
                        (node, msg,
                        ip->interfaceInfo[*interfaceIndex]->manager->
                        predictiveTimeAnalysisTimer);
                    break;
                }
#endif
                // STATS DB CODE
...
//                		NetworkIpEnqueuePktNoRoute(node, newMsg);
//                		Message* timerMsg = MESSAGE_Alloc(node,
//                				NETWORK_LAYER,
//                				NETWORK_PROTOCOL_IP,
//                				MSG_NETWORK_Alert_Route_Packet);
//                		MESSAGE_Send(node,timerMsg,(clocktype) (400 * MILLI_SECOND));
//                		MESSAGE_Free(node,msg);
//                	}
#endif
                    break;
...
#endif
#ifdef ADDON_MIH
                case MSG_NETWORK_Alert_Route_Packet:
                {
                	NetworkDataIp *ip = (NetworkDataIp *)
                	                        		node->networkData.networkVar;
                	ERROR_Assert(ip != NULL, "Unable to extract IP information\n");
...
//                	int *outgoingInterface = NULL;

                	while (!ip->packetBuf->empty())
                	{
                		msgInBuffer = ip->packetBuf->front();
                		ip->packetBuf->pop();
                		ip->stats.numDataPacketNoRouteDeQueue ++;
//                		outgoingInterface = (int *) MESSAGE_ReturnInfo(msgInBuffer);
...
//                			*outgoingInterface = 0;
                		...
                		// update stat
//                		ip->stats.numDataPacketSentToL2 ++;
                	}
                	ip->pktBufSize = 0;
                	break;
                }
       ...
       {
           CesSrwMospfHandleProtocolEvent(node, msg);
           break;
       }
#endif
#ifdef ADDON_MA
...
#ifdef ADDON_MIH
	if (ip->mihFunctionStruct)
	{
		MihFunctionFinalize(node);
	}
	delete ip->packetBuf;
#endif //ADDON_MIH
...
#endif // CYBER_CORE
#ifdef ADDON_BOEINGFCS
        // Print stats for CES_ISAKMP
        if (ip->interfaceInfo[i]->isCES_ISAKMPEnabled == TRUE)
        {
            CES_ISAKMPFinalize(node, i);
        }
#endif
    }
...
#ifdef ADDON_MIH
		//there should be another condition here for case when it is no longer connected to UMTS network
		((!(node->isMihfEnabled)) || (MihGetCurrentPoaMacProtocol(node) == MAC_PROTOCOL_CELLULAR)) &&
#endif
    ...
    // Handle packet to security protocol if IpSec enable over this interface
            ...
            )
        {
            if (IPsecHandleOutboundPacket(
                    node, msg, incomingInterface,
                    outgoingInterface, nextHop, NULL, FALSE, 0))
            {
                return;
            }
        }
        ...
        */
        ...
        isEmulationPacket = (BOOL *) MESSAGE_ReturnInfo(msg,
            INFO_TYPE_IsEmulationPacket);
#endif
    ...
    else
    {
        if (fragHead)
        {
         ...
         // Below is the same code that used for non-fragment message above
                ...
                {
                    if (IPsecHandleOutboundPacket(
                            node, fragHead->msg, incomingInterface,
                            outgoingInterface, nextHop, NULL, FALSE, 0))
                    {
                        tempFH = fragHead;
                        fragHead = fragHead->next;
        ...
        else
        {
            GUI_Receive(node->nodeId,
                ...
                *timing = getSimTime(node) ;
                ...
                HandleStatsDBNetworkSummaryOutUpdate(node, fragHead->msg,
                    nextHop, outgoingInterface); //outgoing
#endif
                HandleStatsDBNetworkOutUpdate(node,
                    fragHead->msg, nextHop,   outgoingInterface) ;
...
BOOL IsMyIsakmpPacket(Node* node, unsigned char ip_p, int incomingInterface)
{
    ...
    if (ip->iahepEnabled && ip->iahepData->nodeType == RED_NODE
     && (ip_p == IPPROTO_ISAKMP || ip_p == IPPROTO_ESP)
...
*/
    ...
    if (ip->interfaceInfo[incomingInterface]->spdIN
        && IPsecRequireProcessing(node, msg, incomingInterface,destinationAddress))
    {
...
//GuiEnd
    ...
    if (NetworkIpIsMulticastAddress(node, ipHeader->ip_dst))
    {
            ...
            multicastProtocolType)
        {
            ...
            case MULTICAST_PROTOCOL_CES_SRW_MOSPF:
            {
                if (CesSrwMospfCheckIfItIsDuplicatePacket(node,
                    incomingInterface, msg))
                {
#ifdef ADDON_DB
                    ...
                    HandleNetworkDBEvents(
                        ...
                        incomingInterface, // use incoming interface here
                        "NetworkPacketDrop",
                        "Duplicate Multicast Packet, MOSPF",
                        0,
                        0,
                        0,
                        0);
    ...
    int interfaceIndex)
{
    ...
    for (i = 0; i < numPriorities; i++)
    {
        ...
        if (!node->networkData.useNetworkCesQosDiffServ) {
            ...
            addedQueuePriority = (*schedulerPtr).addQueue(queue, priority,
                queueWeight[i]);

#if ADDON_DB
                // Add this network output queue to the queue description table
            StatsDb* db = node->partitionData->statsDb;
            ...
            if (db != NULL && db->statsDescTable->createQueueDescTable)
            {
                StatsDBQueueDesc queueDesc(node->nodeId,
                    interfaceIndex,
                    (*schedulerPtr).numQueue()-1,
                    "Network Output");
                queueDesc.SetQueueDiscipline((std::string) queueTypeString);
                queueDesc.SetQueueSize(queueSize);
                int queuePriority =
                    (*schedulerPtr).GetQueuePriority((*schedulerPtr).numQueue()
                    -1);
            ...
            // Initialize Queue depending on queueTypeString specification
             ...
             addedQueuePriority = (*schedulerPtr).addQueue(queue, i,
                 queueWeight[i]);
        ...
        if (db && db->statsDescTable->createQueueDescTable)
        {
            ...
            queueDesc.SetQueueSize(queueSize);
            int queuePriority =
                (*schedulerPtr).GetQueuePriority((*schedulerPtr).numQueue()-1);
            queueDesc.SetQueuePriority(queuePriority);
            queueDesc.m_QueueMetaData = *(queue->meta_data);
...
void NetworkIpJoinMulticastGroup(Node *node,
                          ...
                          clocktype delay)
{
    Int32 interfaceId = ANY_INTERFACE;
    char errStr[MAX_STRING_LENGTH];
...
#endif // ADDON_MAODV
        {
            interfaceAddress = MAPPING_GetDefaultInterfaceAddressFromNodeId(
                                node,
                                node->nodeId);
            interfaceId = MAPPING_GetInterfaceIndexFromInterfaceAddress(
                                node,
                                interfaceAddress);
              ...
              int interfaceIndex, NodeAddress previousHopAddress)
{
        ...
        (node->macData[mappedInterfaceIndex]->isIpneInterface))
    {
        ...
        if ((node->isHitlNode == TRUE)
            && (ipProtocolNumber !=  IPPROTO_OSPF)
            && (ipProtocolNumber !=  IPPROTO_PIM)
            && (ipProtocolNumber !=  IPPROTO_IGMP))
        {
...
//InsertPatch TRANSPORT_SEND_TO_TRANS
        ...
        {
            CesSrwMospfHandleProtocolPacket(node,
                                          msg,
                                          sourceAddress,
                                          destinationAddress,
                                          interfaceIndex,
...
//CES HAIPE
#ifdef ADDON_BOEINGFCS
        case IPPROTO_CES_ISAKMP:
        {
            CES_ISAKMPHandleProtocolPacket(node,
                                       msg,
                                       sourceAddress,
                                       destinationAddress);
            break;
                        ...
                        NodeAddress previousHopAddress)
{
    ...
    if ((incomingInterface != CPU_INTERFACE) &&
        ...
        (node->macData[incomingInterface]->isIpneInterface))
            {
#ifdef HITL_INTERFACE
        if ((node->isHitlNode == TRUE) && (ipHeader->ip_p !=  IPPROTO_OSPF)
...
#ifdef ADDON_BOEINGFCS
        if (!IsIPsecProcessed(node, msg)
            && ip->interfaceInfo[outgoingInterface]->isCES_ISAKMPEnabled
            && ipHeader->ip_src != ANY_DEST
            && ipHeader->ip_dst != ANY_DEST
            && !NetworkIpIsMulticastAddress(node, ipHeader->ip_dst)
            && !NetworkIpIsLoopbackInterfaceAddress(ipHeader->ip_dst)
            && ipHeader->ip_p != IPPROTO_CES_ISAKMP)
        {
            BOOL status = TRUE;
            if (IPsecIsMyIP(node, ipHeader->ip_src))
            {
                status = IsCES_ISAKMPSAPresent(node,
                    ipHeader->ip_src,
                    ipHeader->ip_dst);
                if (status == FALSE)
                {
                    //start ISAKMP Phase-1 exchange here
                    CES_ISAKMPSetUp_Negotiation(node, NULL, NULL,ipHeader->ip_src,
                        ipHeader->ip_dst, INITIATOR, PHASE_1);
                }
            }
            if (status == TRUE)
            {
                CES_ISAKMPNodeConf* nodeconfig = NULL;
                ...
                {
                    //start ISAKMP Phase-2 exchange here
                    CES_ISAKMPSetUp_Negotiation(node, nodeconfig, NULL,
                        ipHeader->ip_src, ipHeader->ip_dst, INITIATOR, PHASE_2);
                }
            }
        }
    ...
    if (db != NULL)
    {
        if (db->statsAggregateTable->createQueueAggregateTable)
        {
            if (node->partitionData->partitionId == 0 &&
                node->partitionData->firstNode == node)
            {
                if (db != NULL) {
                    string tableStr;
                    char dscpbuf[MAX_STRING_LENGTH];
                    tableStr = "ALTER TABLE QUEUE_Aggregate";
            ...
            #if defined(ADDON_DB_USE_QDH)
                    partition->dynamicHierarchy.WriteAsString("/db/write",
                        tableStr);
            #else
                    db->driver->exec(tableStr);
            #endif
            ...
            // make sure all partitions have the same view of aggregate table
            // eventhough only partition 0 creates the table... all other
            // partitions write to it.
            if (node->partitionData->firstNode == node)
            {
                db->queueDbPtr->STATSDB_UpdateQueueAggregateTableForDscp(node,
                    ds);
            }
        }
    }
   ...
   if (outgoingInterface != CPU_INTERFACE)
    ...
    if (node->networkData.useNetworkCesQosDiffServ) {
        ...
        queueIndex = GenericPacketClassifier(scheduler,
            (int) ReturnPriorityForPHB(node, (TosType)
            IpHeaderGetTOS(ipHeader->ip_v_hl_tos_len)));
    }
#else
    queueIndex = GenericPacketClassifier(scheduler,
...
#ifdef ADDON_MIH
    sprintf(buf, "Number of Packets queued due to no route available = %u", stats->numDataPacketNoRouteEnQueue);
    IO_PrintStat(node, "Network", "IP", ANY_DEST, -1 /* instance Id */, buf);
    sprintf(buf, "Number of Packets dequeued due to no route available = %u", stats->numDataPacketNoRouteDeQueue);
    IO_PrintStat(node, "Network", "IP", ANY_DEST, -1 /* instance Id */, buf);
#endif
...
#endif // WIRELESS_LIB
#ifdef ADDON_BOEINGFCS
                else if (strcmp(protocolString, "ROUTING-CES-HSLS") == 0)
                {
                    // Since this is no longer valid, report warning and skip it
                   ERROR_ReportWarning("HSLS should not be used separately anymore!");
                   continue;
                }
#endif // ADDON_BOEINGFCS
                    ...
                    else
{
            ...
            case MULTICAST_PROTOCOL_CES_SRW_MOSPF:
            {
                if (!NetworkIpGetMulticastRoutingProtocol(node,
                                                MULTICAST_PROTOCOL_CES_SRW_MOSPF))
                {
                    CesSrwMospfInit(node, nodeInput, i);
                }
                else
                {
                    NetworkIpUpdateMulticastRoutingProtocolAndRouterFunction(
                    ...
                    if (ip->isIgmpEnable == TRUE)
                    {
                        IgmpSetMulticastProtocolInfo(
                                       node,
                                       i,
                                       &CesSrwMospfLocalMembersJoinOrLeave);
                    }
                }
                break;
            }
        ...
        (retVal && !strcmp(protocolString, "YES")))
    {
            ...
            (ip->iahepData->nodeType == IAHEP_NODE))
        {
            if (retVal)
            {
                ERROR_ReportWarning("ICMP: cannot be enabled on IAHEP nodes!\n");
...
#ifdef ADDON_MIH
	MihFunctionInit(node, nodeInput);
#endif
    ...
    // Initialization of SNMP MIB
#ifdef SNMP_INTERFACE
    NetworkIpInitInterfaceStatsSNMP(node);
#endif
    ...
    switch(type)
    {
        ...
        case IPPROTO_IPIP_CES_SDR:
            *protocolString = "IPPROTO_IPIP_CES_SDR";
            break;
         case IPPROTO_CES_HSLS:
            *protocolString = "IPPROTO_CES_HSLS";
            break;
#endif // ADDON_BOEINGFCS
...
#ifdef ADDON_BOEINGFCS
        case IPPROTO_CES_ISAKMP:
            *protocolString = "IPPROTO_CES_ISAKMP";
            break;

        case NETWORK_PROTOCOL_CES_ISAKMP:
            *protocolString = "CES_ISAKMP";
            break;
#endif
...
BOOL IsDataPacket(IpHeaderType* ipHeader)
{
    if (ipHeader->ip_p == IPPROTO_UDP ||
        ipHeader->ip_p == IPPROTO_TCP
#ifdef ADDON_BOEINGFCS
        || ipHeader->ip_p == IPPROTO_IPIP_ROUTING_CES_MALSR
#endif
        )
    {
        return TRUE;
...
#ifdef EXATA
                if (MESSAGE_ReturnInfo(msg, INFO_TYPE_IsEmulationPacket) != NULL)
                    return TRUE;
                else
                {
#endif
...
#ifdef ADDON_MIH
void NetworkIpEnqueuePktNoRoute(
        Node* node,
        ...
        Message* msg)
{
    ...
    if (ip->pktBufSize + MESSAGE_ReturnPacketSize(msg)
            < 50000 ) //TODO: replace numerical value with macro
    {
        ip->packetBuf->push(msg);
        ip->stats.numDataPacketNoRouteEnQueue ++;
        ip->pktBufSize += MESSAGE_ReturnPacketSize(msg);
    }
    else
    {
            ip->packetBuf->push(msg);
            ip->stats.numDataPacketNoRouteEnQueue++;
            ip->pktBufSize += MESSAGE_ReturnPacketSize(msg);
            while (ip->pktBufSize > 50000) //TODO: replace numerical value with macro
            {
                Message* toFree = ip->packetBuf->front();
                ip->pktBufSize -= MESSAGE_ReturnPacketSize(toFree);
