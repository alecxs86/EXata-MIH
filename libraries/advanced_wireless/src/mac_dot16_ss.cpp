...
void MacDot16SsPrintParameter(Node* node, MacDataDot16* dot16)
{
    ...
    {
        printf("    neighbor scan RSS trigger = %f\n",
         ...
         MacDataDot16* dot16)
{
    ...
    if (wasFound)
    {
        ...
        dot16Ss->para.nbrScanRssTrigger = doubleVal;
    }
    else
    {
        dot16Ss->para.nbrScanRssTrigger =
    ...
    else
    {
		...
		else if (strcmp(stringVal, "CINR") == 0)
		{
			...
			IO_ReadDouble(
							...
							interfaceIndex,
							nodeInput,
							"MAC-802.16e-HANDOVER-CINR-TRIGGER",
							&wasFound,
							&doubleVal);
			...
			{
				dot16Ss->para.hoCinrTrigger = DOT16e_DEFAULT_HO_CINR_TRIGGER;
			}
			...
			{
				dot16Ss->para.hoCinrMargin = DOT16e_DEFAULT_HO_CINR_MARGIN;
			}
...
#ifdef ADDON_MIH/*
        if (dot16Ss->isTimerSendPeriodicReport)
        {
        	MacDot16Ss_Link_Parameters_Report_indication(node,
        			dot16,
        			signalMea,
        			(unsigned char *) dlMap->baseStationId);
        	dot16Ss->isTimerSendPeriodicReport = FALSE;
        }
*/
#endif
    // handle DL-MAP_IEs
...
#ifdef ADDON_MIH
    if (!dot16Ss->firstTimerSent)
    {
    	dot16Ss->timers.timerSendPeriodicReport = MacDot16SetTimer(
    			node,
    			dot16,
    			DOT16_TIMER_SendPeriodicReport,
    			DOT16_TIMER_SEND_PERIODIC_REPORT_INTERVAL,
    			NULL,
    			0);
    	dot16Ss->firstTimerSent = TRUE;
    }
#endif
...
#ifdef ADDON_MIH
// /**
// FUNCTION   :: MacDot16eSsBuildMobHoIndMsg - overloaded for MIH
// LAYER      :: MAC
// PURPOSE    :: Build the MOB-HO-IND mgmt msg
// PARAMETERS ::
// + node      : Node*        : Pointer to node.
// + dot16     : MacDataDot16*: Pointer to DOT16 structure
// + bsId	   : unsigned char*: target PoA ID
// RETURN     :: Message* : Pointer to the MOB-SCN-REQ msg
// **/
static
Message* MacDot16eSsBuildMobHoIndMsg(Node* node, MacDataDot16* dot16, unsigned char* bsId)
{
    MacDot16Ss* dot16Ss = (MacDot16Ss*) dot16->ssData;
    MacDot16MacHeader* macHeader;
    Message* mgmtMsg;
                                   ...
                                   MacDataDot16* dot16)
{
	...
	NodeAddress proposedBsId = MAPPING_GetNodeIdFromInterfaceAddress(node, MacHWAddressToIpv4Address(node, interfaceIndex, &macHWAddr));
	if (proposedBsId == 2)
	{*/
#endif
    Message* regMgntMsg;
...
#ifdef ADDON_MIH
				MacDot16Ss_Link_Up_indication(node, dot16);
                        ...
                        clocktype rxBeginTime)
{
    ...
    Message* returnPtr = NULL;
#ifdef ADDON_MIH
    PhySignalMeasurement* signalMea = (PhySignalMeasurement* ) MESSAGE_ReturnInfo(msg);
#endif    
...
#ifdef ADDON_MIH
	MacDot16SsSendLinkGoingDownIndication(node, dot16Ss);
	//MacDot16Ss_Link_Parameters_Report_indication(node, dot16Ss);
#endif    
                                   ...
                                   PhySignalMeasurement* signalMea)
{
        ...
        dot16Ss->operational)
    {
        ...
        if (dot16Ss->targetBs != NULL)
        {
            ...
            if (
#ifdef ADDON_MIH
            	(dot16Ss->servingBs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_RSSI &&
#endif            
            dot16Ss->targetBs->rssMean > dot16Ss->servingBs->rssMean &&
                ...
                IN_DB(PhyDot16GetRxSensitivity_mW(
                          ...
                          moduCodeType)))
#ifdef ADDON_MIH
                 || (dot16Ss->servingBs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_CINR
                 && dot16Ss->targetBs->cinrMean > dot16Ss->servingBs->cinrMean
            ...
            dot16->numChannels > 1)
        {
                ...
                dot16Ss->hoStatus == DOT16e_SS_HO_None)
            {
                ...
                dot16Ss->servingBs->rssMean <
                	...
                	&& dot16Ss->servingBs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_RSSI)
                	|| (dot16Ss->servingBs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_CINR
                	&& dot16Ss->servingBs->cinrMean < dot16Ss->para.hoCinrTrigger)
#endif                    
                    )
                {
                    ...
                    if (dot16Ss->nbrBsList != NULL)
                    {
                        ...
                        targetBs != NULL &&
                           (targetBs->rssMean - dot16Ss->servingBs->rssMean)
                            ...
                            || (targetBs != NULL &&
                            dot16Ss->servingBs->trigger.triggerType == DOT16e_TRIGEGER_METRIC_TYPE_CINR &&
                            (targetBs->cinrMean - dot16Ss->servingBs->cinrMean)
			...
			MacDot16SsSendLinkGoingDownIndication(node, dot16Ss);
						...
						{/*
							...
							Convert802AddressToVariableHWAddress(node, &macHWAddr, &macAddr);
							int interfaceIndex = MacGetInterfaceIndexFromMacAddress(node, macHWAddr);
							NodeAddress bsId = MAPPING_GetNodeIdFromInterfaceAddress(node, MacHWAddressToIpv4Address(node, interfaceIndex, &macHWAddr));
							if (dot16Ss->bestPoaId != bsId)
							{
								MacDot16SsSendLinkGoingDownIndication(node, dot16Ss);
								Message* hoIndMsg = MacDot16eSsBuildMobHoIndMsg(
											   node,
											   dot16,
											   dot16Ss->servingBs->bsId);
...
void MacDot16SsLayer(Node* node, int interfaceIndex, Message* msg)
{
    ...
    switch (msg->eventType)
    {
...
#ifdef ADDON_MIH
                case DOT16_TIMER_SendPeriodicReport:
                {
                	dot16Ss->isTimerSendPeriodicReport = TRUE;

                	dot16Ss->timers.timerSendPeriodicReport = NULL;
		...
		case MSG_Link_Get_Parameters_request:
			{
				MacDot16SsReceiveLinkGetParametersRequest(node, dot16Ss, msg);
...
case MSG_MIH_Report_Best_PoA:
			{
				...
				dot16Ss->bestPoaId = info->bestPoaId;
#if DEBUG
				{
					char clockStr[MAX_STRING_LENGTH];
					char buff[MAX_STRING_LENGTH];
