...
#ifdef ADDON_MIH
    double nbrScanCinrTrigger;	//CINR level to trigger neighbor scan
    double hoCinrTrigger;		//CINR level to trigger handover
    double hoCinrMargin;		//Margin of CINR level to select new BS
#endif
} MacDot16SsPara;
// /**
...
#ifdef ADDON_MIH
	int numLinkGoingDownIndSent; //for MIH
	int numLink_Parameters_Report_indication;// for MIH 
	int numLink_Up_indication;//for MIH
	int numLinkHandoverIminent;//for MIH 
	int numLinkHandoverComplete;//for MIH
	int numLink_Down_indication;//for MIH
	int numLinkGetParamsConf;//for MIH
#endif
} MacDot16SsStats;
...
typedef struct
{
    ...
    //
#ifdef ADDON_MIH
    Message* timerSendPeriodicReport;
#endif
...
#ifdef ADDON_MIH
    BOOL isTimerSendPeriodicReport;
    BOOL firstTimerSent;
...
// + potentialIncomingPacket : Message* : Potential incoming packet
// RETURN     :: void : NULL
// **/
