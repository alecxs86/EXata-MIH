    ...
    const UmtsNodebRbSetupPara& initNodebPara)
{
    ...
    ERROR_Assert (pendingRab && !pendingRab->nodebResList->empty(),
                    "Cannot find pending RAB info or "
    ...
    UInt64 extraInfoSize = 0)
{
    ...
    {
        if (DEBUG_HO && ueRrc->ueId == DEBUG_UE_ID && 1)
        {
            UmtsLayer3PrintRunTimeInfo(node, UMTS_LAYER3_SUBLAYER_RRC);
...
#ifdef ADDON_MIH
	 //Umts_Link_Up_indication(node,umtsL3,ueRrc);
#endif
         ...
         UmtsRrcUeDataInRnc* ueRrc)
{
        ...
        it ++)
    {
        ...
        // if it is the only one in the active set, skip the
        // remove operation at this point
        if (activeSetSize > 1 &&
        ...
        {
#ifdef ADDON_MIH
//        	int NodeId = bestActiveCell->cellId/16;
#endif
...
/*
#ifdef ADDON_MIH
	Message *mihMsg = MESSAGE_Duplicate(node, msg);
	IpHeaderType *ipHeader = (IpHeaderType *) msg->packet;
	MESSAGE_RemoveHeader(node, msg, IpHeaderSize(ipHeader), TRACE_IP);
	MihHeaderType* hdr = (MihHeaderType*) (MESSAGE_ReturnPacket(mihMsg) + sizeof(TransportUdpHeader));
	MESSAGE_Free(node, mihMsg);
#endif
*/
