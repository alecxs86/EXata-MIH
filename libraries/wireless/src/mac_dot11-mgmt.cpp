...
#endif
{
    ...
    if (MacDot11IsAp(dot11)) {  // ACCESS POINT
    	...
    	DOT11_ManagementVars* mngmtVars =
    	    ...
    	    // Allocate new Disassociation frame

			DOT11_DisassocFrame * disassocFrame =
				(DOT11_DisassocFrame *) MESSAGE_ReturnPacket(msg);
			memset(disassocFrame, 0, sizeof(DOT11_DisassocFrame));

			// Fill in header
			disassocFrame->hdr.frameType = DOT11_DISASSOCIATION;
			disassocFrame->hdr.destAddr = mngmtVars->apAddr;
			disassocFrame->hdr.sourceAddr = dot11->selfAddr;
			disassocFrame->hdr.address3 = mngmtVars->apAddr;
			//for now just reason code 36 implemented
			disassocFrame->reasonCode = DOT11_RC_DISASSOC_STA_HAS_LEFT; //Requested from peer STA as the STA is leaving the BSS (or resetting)
	...
	//---------------------------Power-Save-Mode-Updates---------------------//
			...
			// SEND Power save Information to AP
			if(MacDot11IsStationSupportPSMode(dot11)){
				disassocFrame->hdr.frameFlags |= DOT11_POWER_MGMT;
			}
	//---------------------------Power-Save-Mode-End-Updates-----------------//
    ...
    DOT11_Frame* rxFrame)
{
    ...
    if (MacDot11IsBssStation (dot11)){
            ...
            strcmp(beacon->ssid, dot11->stationMIB->dot11DesiredSSID) == 0 )
        {
            ...
            if (dot11->stationAssociated && apInfo!=NULL
            		&& (apInfo->rssMean <= dot11->thresholdSignalStrength))
            	MacDot11_Send_Link_Going_Down_Indication(node, dot11);
			if (!dot11->firstTimerSent)
    {
...
#ifdef ADDON_MIH
//        MacDot11Ap_Link_Up_indication(node, dot11);
#endif
...
#ifdef ADDON_MIH
//    MacDot11Sta_Link_Up_indication(node, dot11);
#endif
...
#ifdef ADDON_MIH
    DOT11_DisassocFrame * respFrame =
            (DOT11_DisassocFrame *) MESSAGE_ReturnPacket(rxFrame);
#endif
    ...
    if (MacDot11IsAp(dot11)) {  // ACCESS POINT
			...
			ap->apStationList))
		{
			...
			while(next != NULL)
			{
				...
				if(stationItem->data->macAddr == respFrame->hdr.sourceAddr)
				{
					wasFound = TRUE;
					ap->stationCount--;
				  ...
				  MacDot11ApStationListItemRemove(
							 ...
							 ap->apStationList,
							 stationItem);
				  break; //end while loop after deleting item
			...
			if(!wasFound)
				ERROR_ReportError("MacDot11ManagementProcessDisassociate:"
				                          " STA not found in AP List.\n");
			else
			{
				for (int i = 0; i < node->numberInterfaces; i++)
					if (NetworkIpGetUnicastRoutingProtocolType(node, i, NETWORK_IPV4) == ROUTING_PROTOCOL_BELLMANFORD)
					{
			...
			ERROR_ReportWarning("MacDot11ManagementProcessDisassociate:"
							                          " AP List empty. No STA to disassociate.\n");
#else
        ERROR_ReportError("MacDot11ManagementProcessDisassociate:"
                          " AP mode disassociation not implemented.\n");
        result = DOT11_R_NOT_IMPLEMENTED;
#endif
    } else { // STATION
        // TODO: Not supported yet...not sure how to do it
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
        ...
        {
         #ifdef ADDON_MIH
        	mngmtVars->disassocReceived++;
        	MacDot11ManagementProcessDisassociate(node, dot11, rxFrame);
        	break;
    ...
    MacDataDot11* dot11)
{
	...
	// Alloc Association Request frame
		...
		DOT11_FrameInfo* newFrameInfo = NULL;
		// Build it
		if(MacDot11ManagementBuildDisassociateFrame(
			node,
			dot11,
			msg,
			&newFrameInfo) != DOT11_R_OK ) {
				ERROR_ReportWarning("MacDot11ManagementAssociateRequest:"
									"Build disassociate request failed. \n");
		}
		...
		MacDot11ManagementSetState(
			node,
			dot11,
			DOT11_S_M_IDLE); //have to think about that
    ...
    MacDataDot11* dot11)
{
    ...
    {
       #ifdef ADDON_MIH
    	if ((apInfo->lastMeaTime >= mgmtVars->channelInfo->scanStartTime - (1 * SECOND))
#else
    ...
    int result)
{
    ...
    if(scanStatus == DOT11_R_FAILED && dot11->stationAssociated)
    {
    	...
    	if (dot11->associatedAP != NULL)
    		if (dot11->associatedAP->rssMean < dot11->thresholdSignalStrength)
    		{
    ...
    if((scanStatus == DOT11_R_OK && !dot11->stationAssociated))
    {
		...
		Convert802AddressToVariableHWAddress(node, &macHWAddr, &bestAp->bssAddr);
		int interfaceIndex = MacGetInterfaceIndexFromMacAddress(node, macHWAddr);
		NodeAddress proposedApId = MAPPING_GetNodeIdFromInterfaceAddress(node, MacHWAddressToIpv4Address(node, interfaceIndex, &macHWAddr));
		if ((proposedApId == dot11->bestPoaId) && (getSimTime(node) > (clocktype)(1.3 * SECOND))) //7 is just test value
		{	 //HO to proposed AP only if it is the same as indicated by RM
#endif
...
#ifdef ADDON_MIH
	MacDot11StaSendLinkParametersReportIndication (node, dot11);
#endif
    ...
    MacDataDot11* dot11)
{
	...
	{
		MacDot11Sta_Link_Up_indication(node, dot11);
    //this is where routing should be updated
	char clockStr[MAX_STRING_LENGTH];
	TIME_PrintClockInSecond(getSimTime(node), clockStr);
	printf("Handover end time for WiFi node %d is %s sec\n", node->nodeId, clockStr);
	}
#endif
    ...
    int interfaceIndex)
 {
    DOT11_ManagementVars* mngmtVars =
     ...
     if(dot11->stationType != DOT11_STA_IBSS)
     {
        ...
        if (!MacDot11IsAp(dot11))
        {
        	sprintf(buf, "Management disassociate sent = %d",
        			mngmtVars->disassocSend);
        	IO_PrintStat(
				node,
				"MAC",
        ...
        if (MacDot11IsAp(dot11))
        	sprintf(buf, "Management disassociate received = %d",
        	        mngmtVars->disassocReceived);
        	IO_PrintStat(
				node,
				"MAC",
