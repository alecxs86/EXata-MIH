                                  ...
                                  UmtsLayer3Nodeb *nodebL3)
{
    ...
    if (!wasFound ||ipFragUnit < 256 || ipFragUnit > MAX_NW_PKT_SIZE )
    {
...
void UmtsLayer3NodebPrintStats(Node *node, UmtsLayer3Nodeb *nodebL3)
{
	...
	sprintf(
            ...
            nodebL3->stat.numLinkGetParamsConf);
        IO_PrintStat(node, "Layer3", "UMTS NODE B", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Get_Parameters requests received = %d",
            nodebL3->stat.numLinkGetParamsReq);
        IO_PrintStat(node, "Layer3", "UMTS NODE B", ANY_DEST, -1, buf);
