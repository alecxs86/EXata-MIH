...
#define DEBUG_SPECIFIED_NODE_ID 2 // DEFAULT sh oud be 0
                            ...
                            UmtsLayer3Ue *ueL3)
{
   ...
   // disabled for now
    ...
    sprintf(
        buf,
        "Number of PS domain data packet dropped due to no resource = %d",
        ueL3->stat.numPsPktDroppedNoResource);
    IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);
#ifdef ADDON_MIH
	sprintf(
        buf,
        "Number of PS domain data packet dropped due to UE deregistered = %d",
        ueL3->stat.numPsPktDroppedUeDeregistered);
    IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);
#endif
#if 0
    // disabled for now
	...
	sprintf(
            buf,
            "Number of Link_Going_Down indications sent = %d",
            ueL3->stat.numLinkGoingDownIndSent);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Parameters_Report indications sent = %d",
            ueL3->stat.numLink_Parameters_Report_indication);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Up indications sent = %d",
            ueL3->stat.numLink_Up_indication);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Handover_Imminent indications sent = %d",
            ueL3->stat.numLinkHandoverIminent);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Handover_Complete indications sent = %d",
            ueL3->stat.numLinkHandoverComplete);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Down indications sent = %d",
            ueL3->stat.numLink_Down_indication);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Get_Parameters confirms sent = %d",
            ueL3->stat.numLinkGetParamsConf);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);

		sprintf(
            buf,
            "Number of Link_Get_Parameters requests received = %d",
            ueL3->stat.numLinkGetParamsReq);
        IO_PrintStat(node, "Layer3", "UMTS UE", ANY_DEST, -1, buf);
    ...
    UmtsLayer3Ue* ueL3)
{
        ...
        it ++)
    {
            ...
            (*it)->cpichRscp != 0)
        {
...
#ifdef ADDON_MIH
			ecNoVal = (signed char) ((*it)->cpichEcNo);
#endif
            if (DEBUG_MEASUREMENT )
            {
            ...
            numMeas ++;
#ifdef ADDON_MIH
			if (node->isMihfEnabled)
				Umts_Link_Parameters_Report_indication(node, umtsL3, (*it)->cpichRscp, (*it)->cpichEcNo, (*it)->cellId);
#endif
        }
    }
...
#ifdef ADDON_MIH 
				//Umts_Link_Parameters_Report_indication(node, umtsL3);
...
#ifdef ADDON_MIH
	if (node->isMihfEnabled)
		Umts_Link_Up_indication(node,umtsL3);
#endif
    // update stats
    ueL3->stat.numRrcConnSetupCompSent ++;
        ...
        Message* msg)
{
	...
	//umtsL3->HOStartTime=getSimTime(node);
	//TIME_PrintClockInSecond(umtsL3->HOStartTime, clockStr);
	TIME_PrintClockInSecond(getSimTime(node), clockStr);
	printf("Handover start time for UMTS node %d is %s sec\n", node->nodeId, clockStr);
#endif
...
#ifdef ADDON_MIH
	Umts_Link_Down_indication(node,umtsL3);
#endif
...
#ifdef ADDON_MIH
		if ((!(node->isMihfEnabled)) || (ueL3->bestPoaIdMacProtocol == MAC_PROTOCOL_CELLULAR) || !ueRrc->isFirstTimeRegistered)
#endif
    {
        if (ueRrc->timerT308)
        {
        ...
        // update stat
        ueL3->stat.numRrcConnReqSent ++;
		ueRrc->isFirstTimeRegistered = TRUE;
    ...
    int                 msgSize)
{
    ...
    if (ueL3->gmmData->mainState == UMTS_GMM_MS_DEREGISTERED &&
        ueL3->gmmData->deRegSubState ==
            UMTS_GMM_MS_DEREGISTERED_ATTACH_NEEDED)
    {
//if bestPoA not of cellular type, then keep back from registering
#ifdef ADDON_MIH
		//if ((!(node->isMihfEnabled)) || (ueL3->bestPoaIdMacProtocol == MAC_PROTOCOL_CELLULAR))
#endif
                                ...
                                Message *msg)
{
    ...
    else if (ueL3->gmmData->mainState == UMTS_GMM_MS_DEREGISTERED)
    {
		...
		//if bestPoA not of cellular type, then keep back from registering
#ifdef ADDON_MIH
		if (ueL3->bestPoaIdMacProtocol != MAC_PROTOCOL_CELLULAR)
		{
			MESSAGE_Free(node, msg);
			ueL3->stat.numPsPktDroppedUeDeregistered++;
		}
                      ...
                      UmtsLayer3Data *umtsL3)
{
    ...
    delay = RANDOM_nrand(umtsL3->seed) % UMTS_LAYER3_UE_START_JITTER;
...
void UmtsLayer3UeLayer(Node *node, Message *msg, UmtsLayer3Data *umtsL3)
{
    ...
    switch((msg->eventType))
    {
		...
		case MSG_Link_Get_Parameters_request:
			{
				UmtsReceiveLinkGetParametersRequest(node,umtsL3,msg);

				break;
			}
		case MSG_MAC_Link_Action_request:
			{
				UmtsReceiveLinkActionRequest(node, umtsL3, msg);
				break;
			}
		case MSG_MIH_Report_Best_PoA:
			{
				UmtsReceiveBestPoaIdReport(node, umtsL3, msg);
				break;
			}
#endif
