...
#ifdef ADDON_DB
    clocktype *sendTime = (clocktype *)MESSAGE_AddInfo(node,
            packet, sizeof(clocktype), INFO_TYPE_MacStatsDbContent);
    *sendTime = getSimTime(node);
    ...
    HandleMacDBSummary(
        node, packet, dot11->myMacData->interfaceIndex,
        MAC_SendToPhy, MAC_PROTOCOL_DOT11);
...
//--------------------DOT11e-End-Updates---------------------------------//
        ...
        StatsDb* db = node->partitionData->statsDb;
        if (db->statsAggregateTable->createMacAggregateTable &&
                        !MacDot11StationHasManagementFrameToSend(node, dot11))
        {
            dot11->aggregateStats->dataFramesSent++;
            dot11->aggregateStats->dataBytesSent +=
                MESSAGE_ReturnPacketSize(dequeuedPacket);
        }
#endif
    }
    else {
